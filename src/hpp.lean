...
-- lemma next_gen_pres_particles {g} (a : hpp) (h : a = mk_hpp g) : 
--   count_particles' (next_gen a) = count_particles' a :=
-- begin
--   rw h, unfold mk_hpp,
--   simp [
--     next_gen, cautomaton.g, cautomaton.bound, cautomaton.empty, cautomaton.f,
--     cautomaton.neigh
--   ],
--   rw ← @canonical_pres_count, swap 2, unfold mk_hpp,
--   simp [count_particles', count, count_grid],
--   simp [generate_eq_data, (∘)],
--   rw zip_with_const_eq_map,
--   simp, simp [(∘)],
--   generalize eq : {cautomaton.
--     g := g,
--     empty := X,
--     neigh := cautomatons.neumann,
--     bound := sum.inl cautomatons.ext_one,
--     f := λ _x, translation
--   } = aut,
--   generalize eq₁ :
--     list.map (
--         λ x, translation (
--           list.map (@bound_const (vec_grid₀ cellT) _ X ((ext_aut aut).g)) (cautomatons.neumann x)
--         )
--       ) (gip_g ((ext_aut aut).g)) = l,
--   unfold list.count, 
--   repeat { rw ← add_assoc },
--   rw list_countp_append,
--   rw list_countp_append,
--   rw list_countp_append,

--   -- have : ∀n, 2 * n = n + n, by clear_except; omega, 
--   -- repeat { rw this },
--   -- have : ∀n, 3 * n = n + n + n, by clear_except; omega, 
--   -- repeat { rw this },
--   -- have : ∀n, 4 * n = n + n + n + n  , by clear_except; omega, 
--   -- repeat { rw this },
--   -- simp [list.count],
--   -- repeat { rw ← add_assoc },
--   -- repeat { rw list_countp_append },
...
--   -- have : overlaid_by (grid_bounds g) (grid_bounds (ext_aut aut).g),
--   --   admit,   
...
-- lemma gip_g_ext_one {a : hpp} :
--   gip_g (ext_aut a).g = gip ⟨a.g.o.x - 1, a.g.o.y - 1⟩
--                             ⟨(gtr a.g).x + 1, (gtr a.g).y + 1⟩ :=
-- begin
--   admit
--   -- unfold gip_g gip, 
--   -- generalize eqbly : (grid.bl ((ext_aut aut).g)).y = bly,
--   -- generalize eqtry : (gtr ((ext_aut aut).g)).y = try,
--   -- generalize eql : range_pure bly try = l,
--   -- have a₀ : bly < try, from eqbly ▸ eqtry ▸ blgy_lt_trgy,
--   -- have a₁ : bly + 1 ≤ try, by linarith,
--   -- have : ∃l₁ l₂, l = l₁ ++ l₂ ∧
--   --                l₁ = range_pure bly (bly + 1) ∧
--   --                l₂ = range_pure (bly + 1) try,
--   --   from @range_pure_app bly try _ a₀ eql.symm _ (by linarith) a₁,
...
--   --       {
--   --         intros contra,
--   --         subst eqbly, subst eqtry,
--   --         have eqbound : aut.bound = sum.inl cautomatons.ext_one, by finish, 
--   --         rw [next_gen_ext_one_gbl_y] at contra, swap 2, rw ← eq, 
--   --         rw [next_gen_ext_one_gtr_y] at contra, swap 2, rw ← eq,
--   --         simp at contra,
--   --         have : (grid.bl (aut.g)).y < 1 + (gtr (aut.g)).y,
--   --           {
--   --             have : (grid.bl (aut.g)).y < (gtr (aut.g)).y, from blgy_lt_trgy,
--   --             linarith
--   --           },
--   --         linarith
--   --       },
--   --     apply lt_of_le_of_ne a₁ this
--   --   },
--   -- have : bly + 1 ≤ try - 1, by linarith,
--   -- have : ∃l₃ l₄, l₂ = l₃ ++ l₄ ∧
--   --                l₃ = range_pure (bly + 1) (try - 1) ∧
--   --                l₄ = range_pure (try - 1) try,
--   --   from @range_pure_app (bly + 1) try _ a₁ eq₃ _ this (by linarith),
--   -- rcases this with ⟨l₃, ⟨l₄, eq₄, eq₅, eq₆⟩⟩,
-- end
...
--   ],
--   rw ← @canonical_pres_count, swap 2, unfold mk_hpp,
--   simp [count_particles], rw ← map_generate_map_v₀,
--   rw ← map_generate_map_v₀,
--   simp [generate_eq_data, (∘)],
--   rw zip_with_const_eq_map,
--   simp, simp [(∘)],
...
--     f := λ _x, translation
--   } = aut,
--   rw gip_g_ext_one, rw ← eq, simp [expand_gtr, relative_grid.rows, grid.bl, relative_grid.cols],
--   -- generalize eq₁ :
--   --   list.map (
--   --       λ x, translation (
--   --         list.map (@bound_const (vec_grid₀ cellT) _ X ((ext_aut aut).g)) (cautomatons.neumann x)
...
--   --     ) (gip_g ((ext_aut aut).g)) = l,
--   {
--     -- split gip_g with ∃a < b < c!
--   }
-- end

-- theorem generate_pres_count {g} (a : hpp) (h : a = mk_hpp g) {n} :
--   count_particles' a = count_particles' (step_n a n) :=
-- begin
